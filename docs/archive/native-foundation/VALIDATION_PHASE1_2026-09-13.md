...
Phase 1 native plugin working tree:
...
Environment: Linux/WSL2, Python 3.12.12, OMP 18.1.18, Bun 1.3.14.

Current regression evidence:

- `uv run python -m pytest -q tests/test_native_plugin.py`: **4 passed**.
- `uv run python -m pytest -q tests/test_install_harness.py`: **55 passed**.
- `just test`: **125 passed**.
- `just verify`: passed, including source validation, registry checks and
  `git diff --check HEAD`.

Isolated native smoke:

- `omp plugin link /home/han/project/omp-kit` registered package `omp-kit@0.0.0`;
- OMP discovered `luna-code`, `luna-deep`, `luna-doc`, and `sol-review` with no forced
  model;
- OMP discovered all 15 active registry skills, including `document-parser` hidden from
  model listing as required by its explicit-only activation policy;
- OMP discovered `rules/omp-kit-workflow.md` as `alwaysApply` for Main and filtered it
  from a custom subagent;
- agents, skills and rules remained linked plugin resources rather than copied agent-root
  files;
- `omp plugin uninstall omp-kit` removed the isolated registration cleanly;
- no provider/model request occurred and the user's live OMP configuration was untouched.

The pre-native Harness v2 final-review evidence below remains historical. Its retained
SearchFirst record still has incomplete session-init fallback metadata under the stricter
auditor; Phase 1 does not alter that historical limitation.
